...
options_screen.py
-----------------
Settings screen: music volume, move sound volume, and brightness.
...
class OptionsScreen:
    ...
    def __init__(self, screen: pygame.Surface, options: dict):
        ...
        # ── Music volume ──────────────────────────────────────────────────────
        self._vol_dec = Button((cx - 90, 220, 46, 46), "−", self._font_val)
        self._vol_inc = Button((cx + 44, 220, 46, 46), "+", self._font_val)

        # ── Move sound volume ─────────────────────────────────────────────────
        self._move_dec = Button((cx - 90, 360, 46, 46), "−", self._font_val)
        self._move_inc = Button((cx + 44, 360, 46, 46), "+", self._font_val)

        # ── Brightness ────────────────────────────────────────────────────────
        self._bright_dec = Button((cx - 90, 500, 46, 46), "−", self._font_val)
        self._bright_inc = Button((cx + 44, 500, 46, 46), "+", self._font_val)
    ...
    def draw(self) -> None:
        ...
        # ── Music volume ──────────────────────────────────────────────────────
        self._label("MUSIC VOLUME", cx, 192)
        self._vol_dec.draw(self.screen, mouse)
        vol_text = self._font_val.render(f"{self.options['music_volume']}%", True, ACCENT)
        self.screen.blit(vol_text, vol_text.get_rect(center=(cx, 243)))
        self._vol_inc.draw(self.screen, mouse)
        self._draw_bar(cx, 276, self.options["music_volume"], color=(255, 0, 200))

        # ── Move sound volume ──────────────────────────────────────────────────
        self._label("MOVE SOUND VOLUME", cx, 332)
        self._move_dec.draw(self.screen, mouse)
        move_text = self._font_val.render(f"{self.options['move_volume']}%", True, ACCENT)
        self.screen.blit(move_text, move_text.get_rect(center=(cx, 383)))
        self._move_inc.draw(self.screen, mouse)
        self._draw_bar(cx, 416, self.options["move_volume"], color=(0, 255, 240))

        # ── Brightness ──────────────────────────────────────────────────────────
        self._label("BRIGHTNESS", cx, 472)
        self._bright_dec.draw(self.screen, mouse)
        pct = self._font_val.render(f"{self.options['brightness']}%", True, ACCENT)
        self.screen.blit(pct, pct.get_rect(center=(cx, 523)))
        self._bright_inc.draw(self.screen, mouse)
        self._draw_bar(cx, 556, self.options["brightness"], color=(160, 80, 255))
